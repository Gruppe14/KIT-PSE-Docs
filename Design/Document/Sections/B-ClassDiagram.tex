...
The last step done by WebpageControllers is to create a valid HTTP response. %ist http response ein Teil vom handshake
...
%Do we have to specify how one might add them?
...
CAPSLOCK REGELT, UND SO!!11einself %JA, ABER DAS KANN NICHT DA BLEIBEN
...
from the logfile%from the logfile or rows accessed in the original db? %+1, it is not clear
